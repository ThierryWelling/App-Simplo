...
export function EditThankYouPageDialog({ page, isOpen, onClose, onUpdate }: Props) {
  const [isLoading, setIsLoading] = useState(false)
  const { toast } = useToast()
  const [formData, setFormData] = useState<ThankYouPage>({
    ...page,
    colors: {
  ...
  // Atualiza o formData quando a página mudar
  useEffect(() => {
    setFormData({
      ...page,
      colors: {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <CardDescription>Personalize sua página de agradecimento com as informações abaixo</CardDescription>
        </DialogHeader>
                    ...
                    <div className="flex gap-3 mt-1.5">
                      <div className="flex-shrink-0">
                        <Input
                          id="text_color"
                          type="color"
                          value={formData.colors?.text || '#000000'}
                          onChange={e => handleChange('colors', {
                            ...formData.colors,
                            text: e.target.value
                          })}
